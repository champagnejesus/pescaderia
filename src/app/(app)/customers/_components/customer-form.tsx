    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="bg-zinc-900 rounded-[28px] p-5 space-y-4 border border-zinc-800">
        <div className="space-y-1.5">
          ...
          <Input id="name" name="name" required defaultValue={defaultValues?.name} disabled={pending} placeholder="Nombre del cliente" className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600" />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <label htmlFor="address" className="text-sm font-medium text-zinc-400">Dirección</label>
          <Input id="address" name="address" defaultValue={defaultValues?.address ?? ""} disabled={pending} placeholder="Calle y número" className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600" />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="notes" className="text-sm font-medium text-zinc-400">Notas</label>
          <textarea id="notes" name="notes" rows={3} defaultValue={defaultValues?.notes ?? ""} disabled={pending} className="h-auto w-full rounded-xl border border-zinc-700 bg-zinc-800 px-4 py-3 text-base text-white transition-colors focus-visible:border-[#5E5CE6] focus-visible:ring-3 focus-visible:ring-[#5E5CE6]/50 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm resize-none placeholder:text-zinc-600" placeholder="Ej: paga siempre a los 15 días" />
        </div>
      </div>
